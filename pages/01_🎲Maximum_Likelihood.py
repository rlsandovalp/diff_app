...
# 🎲 **Maximum Likelihood**
> Maximum likelihood framework tackles the estimation of model parameters. This procedure define the "true" values of a model as those
> that minimize the difference between measured data and the predictions generated by the model (i.e., minimize the model residuals). In
> theory, the residuals correspond to measurement errors. In practice however, residuals are a combination of multiple sources of error (e.g., 
> measurement errors, analytical model errors, and numerical model errors). Given the numerous sources of error and on the basis of the 
...
As $\varepsilon = x^* - \hat{x}$ ($x^*$ being the measured data and $\hat{x}$ being the values of $x$ estimated with the model), then
...
> The idea of maximum likelihood is to maximize the previous function (likelihood function). Typically, 
> for the optimization procedure it is convenient to work with the logarithm instead of the actual values. 
> Note that the maximum value of the logarithm of the function coincide with the maximum value of the function 
> (by some principle whose name I dont remember). Also, since maximizing is for loosers (Weinberger, 2022), 
...
By considering that the variance of the error is independent of the measurements the optimization procedure reduces to:
...
> If the function is not crazy it is convenient to use the analytical procedure. Such a procedure consist in deriving the loss function 
...
> space of the uncertain parameters. Such a procedure has two drawbacks: (i) There is not guarantee that the optimal parameter values 
> can be found; 
